...
public class EventServices {
    ...
    public void addEvent(Event e) {
        int nbrP=0 ;
        String qry = "INSERT INTO `event` (`name`, `type`, `date`, `desc`, `eventpath`, `localisation`,`nombresP`) VALUES (?,?, ?, ?, ?, ?, ?)";
        try {
  ...
  public List<Event> getEvent() {
    ...
    public void updateEvent(Event e) {
        String qry = "UPDATE `event` SET `name`=?, `type`=?, `date`=?, `desc`=?, `eventpath`=?, `localisation`=?  WHERE `id`=?";
        try {
            PreparedStatement stm = cnx.prepareStatement(qry);
            stm.setString(1, e.getName());
            stm.setString(2, e.getType());
            ...
            stm.setString(5, e.getImagePath());
            stm.setString(6, e.getLocal());
            stm.setInt(7, e.getId()); // Utilisez e.getId() pour définir l'ID de l'événement
            stm.executeUpdate();
        } catch (SQLException ex) {
            throw new RuntimeException(ex);
        }
    ...
    public void deleteEvent(int eventId) {
        try {
            ...
            PreparedStatement deleteReservationsStatement = cnx.prepareStatement(deleteReservationsQuery);
            ...
            PreparedStatement deleteEventStatement = cnx.prepareStatement(deleteEventQuery);
